...
public class DetailSupplyReleaseLogic extends BaseZLogic {

	@Override
	protected boolean execute(ZLogicParam logicParam, ZSimpleJsonObject res, EntityManager em) throws Exception {
		// TODO Auto-generated method stub
		DetailSupplyReleaseParam myParam = (DetailSupplyReleaseParam)logicParam;
		Integer id = myParam.getId();
		
		TbSupplyReleaseDao supplyReleaseDao = new TbSupplyReleaseDao(em);
		TbFileDao tfDao = new TbFileDao(em);
		
		Map<String, Object> result = supplyReleaseDao.detailSupplyRelease(id);
		
		if (result != null) {
			List<TbFile> tfs =tfDao.findBySupplyReleaseId(id);
