...
public class UpdateBrandSaleUpStatusLogic extends BaseZLogic{

	@Override
	protected boolean execute(ZLogicParam logicParam, ZSimpleJsonObject res, EntityManager em) throws Exception {
		UpdateBrandSaleStatusParam myParam = (UpdateBrandSaleStatusParam)logicParam;
		String id =myParam.getId();
		Integer upStatus = StringUtils.isNullOrEmpty(myParam.getStatus())?null:Integer.valueOf(myParam.getStatus());
		...
		List<String> ids=Arrays.asList(id.split(","));
			for(String i:ids){
				...
				tbBrandSale.setUpStatus(upStatus);
