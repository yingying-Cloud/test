...
public class StatisticEnterpriseLogic extends BaseZLogic {

	public StatisticEnterpriseLogic() {
	...
	protected boolean execute(ZLogicParam logicParam, ZSimpleJsonObject res, EntityManager em) throws Exception {
		...
		StatisticEnterpriseParam myParam = (StatisticEnterpriseParam)logicParam;
		...
		Map<String, Object> totalMap = enterpriseDao.statisticEnterpriseTotalByDscdStatus(selectAll,permissionEnterpriseIdList);
		List<Map<String, Object>> resultList = enterpriseDao.statisticEnterpriseByDscdStatus(selectAll,permissionEnterpriseIdList);
	...
	public String export(List<Map<String, Object>> dataList) throws Exception {
		...
		//导出模块
		HSSFSheet targetSheet = targetWork.createSheet("农资店备案情况");
		
		//创建输入流，读取模板文件
		InputStream in = new FileInputStream(TemplateFilePath + "/statisticEnterprise.xls");
        HSSFWorkbook sourceWork = new HSSFWorkbook(in);
		HSSFSheet sourceSheet = sourceWork.getSheet("农资店备案情况");
		...
		for (int i = 0; i < dataList.size(); i++) {
			...
			cell2.setCellValue(orderMap.get("pend")+"");
			cell2.setCellStyle(cs);
			
			HSSFCell cell3 = row.createCell(4);
			cell3.setCellValue(orderMap.get("success")+"");
			cell3.setCellStyle(cs);
			
			HSSFCell cell4 = row.createCell(5);
			cell4.setCellValue(orderMap.get("fail")+"");
