...
public class StatisticOutLogic extends BaseZLogic {

	public StatisticOutLogic() {
	...
	protected boolean execute(ZLogicParam logicParam, ZSimpleJsonObject res, EntityManager em) throws Exception {
		...
		StatisticOutParam myParam = (StatisticOutParam)logicParam;
		...
		List<Object[]> dataList = toolOrderDao.statisticOut(tm, name,dscd,isValidation,selectAll,permissionEnterpriseIdList,uniformPrice);
		...
		for (Map<String, Object> map : enterpriseList) {
			...
			for (String d : dayList) {
				...
				map.put("outNum"+d, orderNum.intValue());
				total += orderNum.intValue();
			...
			map.put("total", total);
			map.put("xsDay", day-i);
			map.put("wxsDay", thisMonthDayUntilNow-(day-i));
	...
	private String export(List<String> dayList,List<Map<String, Object>> enterpriseList,String tm)  throws Exception{
		...
		lockStyle.setLocked(true);
			...
			/* 创建一个sheet */
			HSSFSheet sheet = workbook.createSheet(tm+"销售情况");
			...
			String[] ends = new String[] {"销售量","销售（天）","未销售（天）","使用率"};
			...
			for (int j=0;j<enterpriseList.size();j++) {
				...
				for (int i=0;i<dayList.size();i++) {
					...
					headerCell.setCellValue(enterprise.get("outNum"+dayList.get(i))+"");
				...
				HSSFCell endCell3 = row.createCell(dayList.size()+1+1+1+1);
				HSSFCell endCell4 = row.createCell(dayList.size()+1+1+1+1+1);
				endCell1.setCellValue(enterprise.get("total")+"");
				endCell2.setCellValue(enterprise.get("xsDay")+"");
				endCell3.setCellValue(enterprise.get("wxsDay")+"");
				endCell4.setCellValue(enterprise.get("utilizationRate")+"");
				endCell2.setCellStyle(lockStyle);
				endCell1.setCellStyle(lockStyle);
				endCell3.setCellStyle(lockStyle);
				endCell4.setCellStyle(lockStyle);
			}
