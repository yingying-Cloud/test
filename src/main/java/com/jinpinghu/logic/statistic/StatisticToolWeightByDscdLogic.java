...
public class StatisticToolWeightByDscdLogic extends BaseZLogic {

	public StatisticToolWeightByDscdLogic() {
	...
	protected boolean execute(ZLogicParam logicParam, ZSimpleJsonObject res, EntityManager em) throws Exception {
		...
		List<Map<String, Object>> resultList = statisticDao.statisticToolWeightByDscd(selectAll,permissionEnterpriseIdList,null,"1",startMonth,endMonth);
		List<Map<String,Object>> total = statisticDao.statisticToolWeightByDscd(selectAll,permissionEnterpriseIdList,null,null,startMonth,endMonth);
		if(total!=null) {
			total.get(0).put("NAME", "合计");
		}
		if(resultList != null)
		...
		res.add("status", 1).add("msg", "操作成功").add("result", total);
		return true;
	}
	public String export(List<Map<String, Object>> dataList,String startMonth,String endMonth) throws Exception {
		...
		//导出模块
		HSSFSheet targetSheet = targetWork.createSheet("农资重量统计");
		
		//创建输入流，读取模板文件
		InputStream in = new FileInputStream(TemplateFilePath + "/statisticToolWeight.xls");
        HSSFWorkbook sourceWork = new HSSFWorkbook(in);
		HSSFSheet sourceSheet = sourceWork.getSheet("农资重量统计");
		...
		if (!StringUtils.isEmpty(startMonth) && !StringUtils.isEmpty(endMonth)) {
			targetSheet.getRow(0).getCell(0).setCellValue(startMonth+"至"+endMonth+"农资重量统计");
		}else {
			targetSheet.getRow(0).getCell(0).setCellValue("农资重量统计");
		}
		...
		for (int i = 0; i < dataList.size(); i++) {
			...
			cell1.setCellValue(new BigDecimal(orderMap.get("ng")+"").divide(d).setScale(2,BigDecimal.ROUND_HALF_UP)+"");
			cell1.setCellStyle(cs);
			
			HSSFCell cell3 = row.createCell(2);
			cell3.setCellValue(new BigDecimal(orderMap.get("fg")+"").divide(d).setScale(2,BigDecimal.ROUND_HALF_UP)+"");
