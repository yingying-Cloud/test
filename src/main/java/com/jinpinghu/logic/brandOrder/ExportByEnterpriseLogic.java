...
//		}
		...
		List<Object[]> orderList = tbBrandDao.findAllOrder(startTime,endTime,enterpriseName);
		...
		if(orderList != null) {
			for (int i = 0; i < orderList.size(); i++) {
				...
				orderResultMap.put("brandName", order[7] == null ? "" : order[7].toString());

				List<Object[]> orderInfoList = tbBrandDao.findOrderInfo(Integer.valueOf(order[6].toString()));
				...
				if(orderInfoList != null) {
					for (int j = 0; j < orderInfoList.size(); j++) {
						...
						orderInfoResultMap.put("brandName", orderInfo[0]==null?"":orderInfo[0].toString());
	...
	public String export(List<Map<String, Object>> dataList) throws Exception {
		...
		for (int i = 0; i < dataList.size(); i++) {
			...
			cell0.setCellValue((String)orderMap.get("brandName"));
