...
public class IDUtils {
	...
	public static void main(String[] args) {
		...
		try {
			...
			List<Map<String, Object>> findByiv = areaDao.findByiv();
			for(Map<String, Object> l:findByiv) {
				Object legalPersonIdcard = l.get("legalPersonIdcard");
				Object id = l.get("id");
				boolean idNumber = isIDNumber(legalPersonIdcard.toString());
				TbLinkOrderInfo info = tbLinkOrderInfoDao.findById(Integer.valueOf(id.toString()));
				if(info!=null) {
				if(idNumber) {
						info.setIsValidation(1+"");
					}else {
						info.setIsValidation(2+"");
					}
				tbLinkOrderInfoDao.update(info);
				}
			}
